...
public sealed class TransactionService : ITransactionService
{
    ...
    public async Task<List<TransactionDto>> GetTransactionsAsync(int month, int year)
    {
        await AddAuthorizationHeaderAsync();

        var response = await _httpClient.GetAsync($"/api/v1/transactions?month={month}&year={year}");
        return await ReadContentAsync<List<TransactionDto>>(response);
    }

    public async Task<MonthlySummaryDto> GetSummaryAsync(int month, int year)
    {
        await AddAuthorizationHeaderAsync();

        var response = await _httpClient.GetAsync($"/api/v1/transactions/summary?month={month}&year={year}");
        return await ReadContentAsync<MonthlySummaryDto>(response);
    }
    ...
    public async Task<TransactionDto> CreateTransactionAsync(CreateTransactionRequest request)
    {
        ...
        return await ReadContentAsync<TransactionDto>(response);
    }
    ...
    private static async Task<T> ReadContentAsync<T>(HttpResponseMessage response)
    {
        ...
        // Try to deserialize wrapped ApiResponse<T> first
        try
        {
        ...
        {
            // Ignore and fall back to direct deserialization of T.
        }
